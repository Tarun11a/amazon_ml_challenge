...
def first_two_tokens(text: str) -> str:
    """First two whitespace-delimited tokens, joined by a single space.
    A slightly tighter blocking key than first_token alone — e.g.
    "amazon india" instead of just "amazon" — trading a little recall
    for a smaller block size. Falls back to whatever tokens exist if
    there's only one (or zero)."""
    text = text.strip()
    if not text:
        return ""
    tokens = text.split()
...
def first_three_char(text: str) -> str:
    """First three characters of an already-normalized string (no spaces
    stripped first, so this doubles as a coarse prefix key). Typo-tolerant
    fallback blocking key: catches near-matches whose first word doesn't
    tokenize the same way but which share a character prefix."""
    text = text.strip().replace(" ", "")
    return text[:3] if text else ""


def address_prefix(address: str) -> str:
    """First two tokens of an already-normalized address — typically the
    house/plot number plus the start of the street name (e.g. "123 mg"
    from "123 mg road near sbi atm bangalore"). Used as a coarse address
    blocking key."""
